...
export function TasksPage(){
  ...
  return <PageFrame eyebrow="РАБОТА МЕНЕДЖЕРОВ" title="Задачи" description="Система создаёт задачи автоматически после получения контакта и пересчитывает приоритет по поведению клиента.">
      ...
      {columns.map(column=>{
        ...
        return <div className="kanbanCol" key={column.status}>
          ...
          {list.length===0?<div className="miniEmpty">Нет задач</div>:<div className="taskStack">{list.map(task=><article className={"taskCard "+task.priority} key={task.id}>
            <div className="taskTop"><span>{task.priority==="urgent"?<Flame size={14}/>:<Clock3 size={14}/>} {priorityName(task.priority)}</span><b>{leadName(task.leadId)}</b></div>
            <h3>{task.title}</h3>
            <p>{task.reason}</p>
