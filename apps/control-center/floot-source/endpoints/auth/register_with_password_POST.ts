...
export async function handle(request: Request) {
  try {
    ...
    // First registered account becomes the workspace admin.
    // The advisory lock makes that decision atomic if two signups race.
    const newUser = await db.transaction().execute(async (trx) => {
      await sql`select pg_advisory_xact_lock(918271)`.execute(trx);
      const countRow = await trx
        .selectFrom("users")
        .select(({ fn }) => fn.countAll<number>().as("count"))
        .executeTakeFirst();
      if(Number(countRow?.count ?? 0)>0){
        throw new Error("Регистрация закрыта. Войдите в существующий аккаунт администратора.");
      }
